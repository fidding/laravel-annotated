...
abstract class Model implements ArrayAccess, Arrayable, Jsonable, JsonSerializable, QueueableEntity, UrlRoutable
{
    ...
    {
        // 如果对象中的$table属性存在则使用作为表名, 否则以模型类名的复数作为数据表名
    ...
    {
        // 魔术方法
        // 动态设置model attribute数据
        // 如 $model->title = 'title_name' 则$key=title , $value=title_name
        $this->setAttribute($key, $value);
    }
    ...
    public function __call($method, $parameters)
    {
        ...
        if (in_array($method, ['increment', 'decrement'])) {
            // 如果调用方法为increment或decrement则引导执行该方法
            // 如User::find($userId)->increment('review');
            // 如User::find($userId)->decrement('review');
            return $this->$method(...$parameters);
        }

        // 调用$this->newQuery()对象的$method方法并带上参数$paramters
        // 如调用(new User())->all()将执行对象的all()方法
    ...
    {
        // 魔术方法, 当调用一个不存在的静态方法时执行
        // 如 User::all()
        // (new static)使用后期静态绑定, 即实例化调用对象, 如User::all()将会实例化User对象
